...
RSpec.describe Dodo::Runner do
  ...
  let(:window) do
    window = Dodo::Window.new(duration) {}
  ...
  describe '#call' do
    ...
    subject { runner.call window, start, enum_opts }
    before do
      allow(window).to receive(:enum).and_return moments
      allow(runner).to receive(:occuring_at).and_yield
    end
    ...
    it 'should make a call to occuring_at once for each yielded moment' do
    ...
    it 'should invoke moment.call once for each moment' do
      moments.map do |moment|
        expect(moment).to receive(:call).with(no_args)
      end
      subject
    end
